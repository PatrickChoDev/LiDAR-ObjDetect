...
def box3d_vol(corners):
    ...
    a = torch.sqrt(((corners[0, :] - corners[1, :]) ** 2).sum())
    b = torch.sqrt(((corners[1, :] - corners[2, :]) ** 2).sum())
    c = torch.sqrt(((corners[0, :] - corners[4, :]) ** 2).sum())
    return a * b * c
...
def box3d_iou(corners1, corners2):
    ...
    with torch.no_grad():
        ...
        area1 = poly_area(np.asarray(rect1)[:, 0], np.asarray(rect1)[:, 1])
        area2 = poly_area(np.asarray(rect2)[:, 0], np.asarray(rect2)[:, 1])
